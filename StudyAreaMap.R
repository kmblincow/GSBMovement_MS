...
base <- ggplot() +
  ...
  geom_point(data = bigd, aes(x = Longitude, y = Latitude),
             size = 2.5, shape = 21, color = "black", fill = "gray20") +
  geom_point(data = stations, aes(x = mean(Longitude), y = mean(Latitude)),
             shape = 22, fill = NA, size = 6) +
  theme(panel.border = element_rect(colour = "black", fill=NA)) +
  coord_cartesian(xlim = c(-135, -112), ylim = c(20,36)) +
...
zoom <- ggplot() +
  ...
  scale_color_manual(values = c("#9a5ea1",
                                "#98823c")) +
  labs(x = "", y = "") +
  coord_cartesian(xlim = c(-117.35, -117.2), ylim = c(32.78, 32.93)) +
  ...
  annotate("text", y= 32.89223,	x = -117.275, label = "Torrey Pines") +
  theme_classic() +
...
png(file="Figures/2022Figs/StudyMap.png",
    width = 3200,
    height = 2500,
    res = 300)
